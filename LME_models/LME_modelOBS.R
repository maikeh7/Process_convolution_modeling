...
make_kmats = function(num_years=30, kernel_SD){
  ...
  bigmat = rbind(newkmat, newkmat, newkmat, newkmat, newkmat,
                 newkmat, newkmat, newkmat, newkmat, newkmat,
                 newkmat, newkmat, newkmat, newkmat, newkmat,
                 newkmat, newkmat, newkmat, newkmat, newkmat,
                 newkmat, newkmat, newkmat, newkmat, newkmat,
                 newkmat, newkmat, newkmat, newkmat, newkmat)
...
##############################################
traindf = read.csv("OBS_ALL_Future.csv")
...
# response is meanStationTMAX minus WRF long term daily mean trend and seasonal trend (from seasonal PC model)
#delta = traindf$meanStationTMAX - traindf$Long_term_trend_fut - traindf$Seasonal_trend_obs_PC
delta = traindf$tmax_norm_VAR1
#kmats180 = make_kmats(kernel_SD = 180)
#kmats90 = make_kmats(kernel_SD =  90)
#kmats60 = make_kmats(kernel_SD = 60)
#kmats30 = make_kmats(kernel_SD = 30)
#kmats14 = make_kmats(kernel_SD = 14)
...
#long term K
# make sure these are the same order as in model call!
...
# no weighting here!
#vf = traindf$Var_scaling_factors_obs^2 
...
end_time <- Sys.time()
end_time - start_time
saveRDS(fit2train, "OBS_1976_2005.Rds")

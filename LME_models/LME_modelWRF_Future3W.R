...
traindf = filter(traindf, YEAR %in% 2037:2067)
...
# response is meanWRF minus WRF long term daily mean trend and seasonal trend (from seasonal PC model)
# delta = traindf$meanWRF - traindf$Long_term_trend_fut - traindf$Seasonal_trend_wrf_PC
...
#weight by seasonal SD (lme works w/ variance so need to square here)
...
end_time <- Sys.time()
end_time - start_time
#saveRDS(fit2train, "WRF3_normvar1.Rds")
saveRDS(fit2train, "WRF_2037_2067.Rds")
